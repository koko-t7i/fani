...
SQLite at `<repo>/<data_dir>/fani.db` remains the sole fani-owned state authority. Native databases upgrade to schema 4 with a no-clobber pre-upgrade backup; stop all old processes first and follow the [upgrade/restore procedure](docs/architecture/native-i18n.md#schema-4-upgrade-and-restore).

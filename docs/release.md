...
fani releases are Linux-first and currently support Intel/AMD 64-bit Linux with glibc 2.31 or newer (Rust target `x86_64-unknown-linux-gnu`). Pushing a SemVer tag runs the cargo-dist 0.32.0 workflow in [`.github/workflows/release.yml`](../.github/workflows/release.yml). The workflow builds the release binary, creates a `.tar.xz` archive and SHA-256 sidecar, generates a CycloneDX XML SBOM with cargo-cyclonedx 0.5.9, validates the archive, attests the final release assets through GitHub artifact attestations, and then creates the GitHub release.
